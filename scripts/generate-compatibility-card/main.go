...
func projectRoot() string {
	if override := os.Getenv("DEARROW_ROOT"); override != "" {
		return override
	}
...
func validateEntries(name string, entries []entry) error {
	...
	for index, item := range entries {
		if field(item, "label") == "" {
			return fmt.Errorf("%s entry %d has no label", name, index+1)
		}
		if _, ok := statuses[field(item, "status")]; !ok {
			return fmt.Errorf("%s entry %d has unsupported status %q", name, index+1, field(item, "status"))
		}
	}
	return nil
}
...
func render(data config) string {
	...
	parts := []string{
		...
		`<desc id="description">DeArrow compatibility information and status legend.</desc>`,
		`<rect width="100%" height="100%" rx="26" fill="#121316" stroke="#2a2d33" stroke-width="2"/>`,
	}
	for index, row := range data.rows {
		column := math.Floor(float64(index) / rowsPerColumn)
		...
		parts = append(parts,
			...
			fmt.Sprintf(`<text x="%.0f" y="%.0f" fill="#98989f" font-family="-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif" font-size="13">%s</text>`, x+24, y+48, html.EscapeString(value)),
